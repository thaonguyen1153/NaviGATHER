...
class ResumeParser:
    ...
    def parse_job_history(self, resume_segment):
        log.info("Parse job history:")
        log.info(resume_segment)
        idx_job_title = self.get_job_titles(resume_segment)
        log.info("Index job title:")
        log.info(idx_job_title)
    ...
    def get_job_titles(self, resume_segment):
        ...
        return idx_line
    # Parse job history by using reg
    def parse_job_history_notitle(self, resume_segment):
        log.info("Parse job history:")
        log.info(resume_segment)
        idx_job_title = self.extract_jobhistory_bysegment(resume_segment)
        log.info("Index job title:")
        log.info(idx_job_title)
        current_and_below = False
        #if not len(idx_job_title):
        ...
        #if idx_job_title[0][0] == 0: current_and_below = True
        job_history = []
        for date, company, position in idx_job_title:
            job_info = {}
            job_info["Date"] = date
            #if summary:
    ...
    def extract_jobhistory_bysegment(self, segment):
